...
namespace Scarecrow
{
    public class Vector3Drawer : MaterialPropertyDrawer
    {
        ...
        public Vector3Drawer(float type)
        {
            _typeIndex = (int)type;
            _isAlwaysShow = true;
        }
        public Vector3Drawer(float type, params string[] showList)
        {
            _typeIndex = (int)type;
            _showList = showList;
        ...
        public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
        {
            ...
            Rect butRect = new Rect(position)
            {
                ...
                width = (_isEditor && _typeIndex == 0) ? 30f : 60f,
                height = 18f
            };
            _isEditor = GUI.Toggle(butRect, _isEditor, "Set", "Button");

            if(_isEditor && _typeIndex == 0)
            {
                Rect barRect = new Rect(position)
                {
                    x = butRect.x + butRect.width + 3f,
                    y = butRect.y,
                    width = 30f,
                    height = 18f
                };
                _autoRotate = GUI.Toggle(barRect, _autoRotate, "A", "Button");
            }
        ...
        private void CreateHandle(Vector3 value)
        {
            ...
            _rotation = Quaternion.FromToRotation(Vector3.forward, value);
            _position = value;
        ...
        private void HandleDraw(SceneView sceneView)
        {
            ...
            if(_typeIndex == 0)
            {
                ...
                if(_autoRotate)
                    _rotation = _rotation * Quaternion.AngleAxis(Time.deltaTime*20, Quaternion.Inverse(_rotation) * Vector3.up);
            ...
            {
                Quaternion rot = _selectGameObj.transform.rotation;
                Vector3 pos = _selectGameObj.transform.position;

                //绘制坐标控件
                _position = Handles.PositionHandle(pos + _position, rot) - pos;

                
                _property.vectorValue = new Vector4(_position.x, _position.y, _position.z, _property.vectorValue.w);

                //绘制手柄中心图标
                Handles.color = Color.green;
                float size = HandleUtility.GetHandleSize(pos) * 0.5f;
                Handles.CubeHandleCap(0, pos + _position, rot, size, EventType.Repaint);
            }
        }
    }
}
