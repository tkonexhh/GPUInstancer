...
namespace Scarecrow
{
    ...
    public class Enum_SwitchDrawer : MaterialPropertyDrawer
    {
        ...
        public override void Apply(MaterialProperty prop)
        {
            ...
            if (prop.type == MaterialProperty.PropType.Float)
            {
                ...
                SetEnumKeyword(prop);
                SetEnumSwitch(prop);
                SetupRenderMode(prop);
            }
        }

        private void SetupRenderMode(MaterialProperty prop)
        {
            // Debug.Log(prop.name + " " + _enumList[_optionIndex]);
            if(prop.name != "__RenderMode")
                return;

            foreach (Material material in prop.targets)
            {
                switch((RenderMode)_optionIndex)
                {
    ...
    public class NonSwitchDrawer : MaterialPropertyDrawer
    {
        //该控件会在以下选项中显示
        private string[] _showList = new string[0];
        //绘制折叠页的ShaderGUI
        ...
        //填写任意数量的选项，当其中一个选项被选中时，该控件会被渲染
        public NonSwitchDrawer() { }
        public NonSwitchDrawer(params string[] showList)
        {
            _showList = showList;
        }
